...
|---|---|---|
| [favicon-enrichment](2026-07-14-favicon-enrichment/) | Background `IHostedService` resolving each site's favicon into `FaviconUrl` | The project's first hosted worker. All 434 synced bookmarks currently have `faviconUrl: null` — the dashboard's icons are the browser guessing from the origin |
| [extension-auto-sync](2026-07-14-extension-auto-sync/) | MV3 service worker syncing on a `chrome.alarms` schedule, badge + error state | Groundwork done: `src/lib/sync.ts` is UI-free so the worker can import `runSync()` as-is, and a re-sync is proven to be a safe no-op (0 created / 260 skipped) |
...
- ⚠️ **Flaky integration suite (~1 full run in 8).** `SearchVectorTests` fails intermittently with no
  ...
  classes run in parallel, each spinning up its own PostgreSQL container. Suggested fix: one xUnit
  collection with `ICollectionFixture<PostgresApiFactory>` so they share a container. Detail in
  [full-text-search/tasks.md](2026-07-14-full-text-search/tasks.md).
